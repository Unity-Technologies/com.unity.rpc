...
namespace Unity.Rpc.Tests
{
    ...
    public class Helpers
    {
        public static Configuration GetConfiguration(int port, int protocol) =>
            new Configuration { Port = port, ProtocolVersion = RpcVersion.Parse(protocol.ToString()) };

        public static ITask<RpcServer> NewServer<T>(ITaskManager taskManager, Configuration configuration, CancellationTokenSource cts)
            where T : class, new()
        {
            return new TPLTask<RpcServer>(taskManager, async () => {
                var inst = new T();
                var server = new RpcServer(configuration, cts.Token).RegisterLocalTarget(inst);
                await server.Start();
                return server;
            }).Start();
        }
            ...
            where T : class
        {
            return new TPLTask<IRequestContext>(taskManager, async () => {
                var client = new RpcClient(configuration, cts.Token).RegisterRemoteTarget<T>();
                await client.Start();
                return client;
            }).Start();
        }
    ...
    public partial class ClientServerConnectionTests : BaseTest
    {
        ...
        public void SingleClientNoServer()
        {
            using (var test = StartTest())
            {
                var client = new RpcClient(new Configuration());
