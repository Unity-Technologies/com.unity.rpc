...
namespace Unity.Ipc.Tests
{
    ...
    public class BasicMathService : IpcServerServiceBase<MathSession>
    {
        [JsonRpcMethod]
    ...
    public class MathSession : ServerSession
    {
    ...
    public class ClientServerConnectionTests : IClassFixture<GetUniqueValues>
    {
        ...
        private readonly IIpcClientLogger _clientLogger;
        ...
        public async void SingleClientServerHandshake()
        {
            ...
            using (var server = new IpcServer<BasicMathService, MathSession>(fileMutexPath, _serverLogger))
            {
                ...
                var version = await clientA.ConnectLocalTcp(serverName, tcpPort, protocolRevision);

                Assert.Equal(1, version.Major);
                Assert.Equal(0, version.Minor);
                Assert.Equal(5, version.Build);
                Assert.Equal(protocolRevision, version.ProtocolRevision);

                var clientB = new IpcClient(fileMutexPath, _clientLogger);
                version = await clientB.ConnectLocalTcp(serverName, tcpPort, protocolRevision);
               ...
               // Thread.Sleep(2000);
                ...
                var rm = await clientA.JsonRpcClient.SendRequestAsync("add", JToken.FromObject(new { opA = 9, opB = 11 }), CancellationToken.None);
                Assert.Null(rm.Error);
                Assert.Equal(20, rm.Result.ToObject<int>());

                var val = await clientA.ExecRequest<int>("add", new {opA = 3, opB = 4}, CancellationToken.None);
                Assert.Equal(7, val);

                rm = await clientB.JsonRpcClient.SendRequestAsync("add", JToken.FromObject(new { opA = 3, opB = 12 }), CancellationToken.None);
                Assert.Null(rm.Error);
                Assert.Equal(15, rm.Result.ToObject<int>());

                rm = await clientA.JsonRpcClient.SendRequestAsync("add", JToken.FromObject(new { opA = 12, opB = 33 }), CancellationToken.None);
                Assert.Null(rm.Error);
                Assert.Equal(45, rm.Result.ToObject<int>());

                rm = await clientB.JsonRpcClient.SendRequestAsync("add", JToken.FromObject(new { opA = 13, opB = 15 }), CancellationToken.None);
                Assert.Null(rm.Error);
                Assert.Equal(28, rm.Result.ToObject<int>());
        ...
        {
            var client = new IpcClient(GetTemporaryDirectory(),_clientLogger);
            var tcpPort = _uniqueValues.NextTcpPort();

            var ex = await Assert.ThrowsAsync<IpcServerNotFoundException>(() => client.ConnectLocalTcp(_uniqueValues.NextServerName(), tcpPort, 0));
        }
